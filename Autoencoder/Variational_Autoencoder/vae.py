...
def generate(n_latent):
    randoms = [np.random.normal(0, 1, n_latent) for _ in range(10)]
    imgs = decoder()

def generate_samples(z):
    image = random.randint(0,128)
    feature = random.randint(0,20)
    latent_sample = z[image, feature]
    output = model.decode(latent_sample)
    plt.imshow(output, cmap='gray', interpolation='interpolation')
